<template>
    <section>
        ...
        <!--列表-->
        <el-table :data="users" @selection-change="selsChange" highlight-current-row style="width: 100%;"
        ...
        <!--编辑界面-->
        <el-dialog :close-on-click-modal="false" :visible.sync="userFormVisible" title="新增或编辑">
            <el-form :model="userForm" :rules="editFormRules" label-width="80px" ref="userForm">
                <el-form-item label="员工名称" prop="username">
                    <el-input auto-complete="off" v-model="userForm.username"></el-input>
                </el-form-item>
                <el-form-item label="员工密码" prop="password">
                    <el-input auto-complete="off" v-model="userForm.password"></el-input>
                </el-form-item>
                <el-form-item label="年龄" prop="age">
                    <el-input auto-complete="off" v-model="userForm.age"></el-input>
                </el-form-item>
                <el-form-item label="邮箱" prop="email">
                    <el-input auto-complete="off" v-model="userForm.email"></el-input>
                </el-form-item>
                <el-form-item label="加密参数" prop="salt">
                    <el-input auto-complete="off" v-model="userForm.salt"></el-input>
                </el-form-item>
        ...
        methods: {
            ...
            handleDel: function (index, row) {
                this.$confirm('确认删除该记录吗?', '温馨提示', {
                    ...
                    //NProgress.start();
                    let para = {id: row.id};
                    this.$http.delete("/user/" + para.id).then(res => {
                        res = res.data
                        if (res.success) {
                            this.$message({
                                message: res.message,
